...
module powerbi.extensibility.visual {
    ...
    export class Visual implements IVisual {
        ...
        public update(options: VisualUpdateOptions) {
            ...
            //Calculated following 4: Out of which 2 are finally used: 3.cumulativeAngularWidth_Array and 4.angularTickLabels_angularPosition_Array
            ...
            console.log("3.&&angularTickLabels_angularPosition_Array", this.angularTickLabels_angularPosition_Array);            
            ...
            for (let angularAxisFactorItem_index in this.angularAxisFactorLevels){ //0-based angularAxisFactorItem_index is number in string type: "0", "1", "2", "3", ..
                for (let radialAxisFactorItem_index in this.radialAxisFactorLevels){ //0-based radialAxisFactorItem_index is number in string type: "0", "1", "2", "3", ..
                    ...
                    /*
                    if(angularAxisFactorItem_index == "0" || angularAxisFactorItem_index == "1"){
                        this.offset = 0.1;
                        this.start_CoordinateRange_x = this.offset;
                        this.end_CoordinateRange_x = 1 - this.offset;
                    }
                    else{
                        this.offset = 0.05; //0.05
                        this.start_CoordinateRange_x = this.offset;
                        this.end_CoordinateRange_x = 1 - this.offset;
                    }
                    */
                ...
                this.slider.append("line")
                    ...
                    .call(
                        d3.behavior.drag().on("drag", function(d){
                            ...
                            d3.select(".handle").attr("cx", currentValue_settled); //.attr("cx", x_playAxisScale( x_playAxisScale.invert(currentValue) ));
                            d3.select(".labelStart").text(formatDate(x_playAxisScale.invert(currentValue_settled) )).attr("x", x_playAxisScale( x_playAxisScale.invert(currentValue_settled) )) //.attr("transform", "translate(" + currentValue.toString() + "," + (-25) + ")")
                    ...
                    .on("click", function() {
                        ...
                        if (sliderStartDate.getTime() == sliderEndDate.getTime()){
                            ...
                            timer = setInterval(
                                function(){
                                    console.log("x/currentValue, text, moving====>", x_playAxisScale( x_playAxisScale.invert(currentValue) ), playButtonText.text(), moving);
                                    d3.select(".handle").attr("cx", x_playAxisScale( x_playAxisScale.invert(currentValue) ));
                                    d3.select(".labelStart").attr("x", x_playAxisScale( x_playAxisScale.invert(currentValue) ))
